...
| CF-217A | Release | `install.sh` にRust binary install pathと `codefire-py` fallback配置を実装する | CF-217 | todo | default `codefire` = Rust binary; Python fallback remains as `codefire-py`; explicit fallback removal only |
| CF-217B | Release | README/runbook/install smokeをRust defaultへ更新する | CF-217A | todo | `./install.sh --prefix <tmp>` installs `codefire` and `codefire-py`; completion generation works |
