...
| CF-213A | Rust Security | Rust commit HMAC signature write/verifyとkey rotation policyを実装する | CF-213 | todo | Python v0.2 `CODEFIRE_SIGNING_KEY`, `--signer`, `--key-id`, `commit_signatures.keys`, `require_history` がoracle |
| CF-213B | Rust Security | Rust remote request HMAC signature、timestamp skew、nonce replay cacheを実装する | CF-213A | todo | Python v0.2 `CODEFIRE_REQUEST_SIGNING_KEY`, `--request-key-id`, `request_signatures.*` がoracle |
