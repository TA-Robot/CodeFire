...
| CF-164 | UX | fire解消のinteractive / batch UXを設計する | CF-163 | done | `extinguish --interactive` plan schema; `--batch`; `--all-matching`; `--edit-rationale`; recent evidence candidates |
...
| CF-166 | Observability | `status` / `scan` / `verify` のphase別時間とAtom/object数を表示するmetrics導線を追加する | CF-163 | done | `crates/codefire-cli/src/metrics.rs`; `status/scan/verify --metrics`; JSON `data.metrics`; `metrics_attach_to_status_scan_and_verify_data`; live metrics smoke |
| CF-167 | Storage | object store size report、large artifact warning、external artifact reference方針を実装/文書化する | CF-166 | done | `storage report`; large_object warning; external artifact refs documented; `storage_report_counts_objects_by_type_and_warns_large_objects`; `evidence_add_records_artifact_ref_and_command_capture`; live storage/evidence smoke |
...
| CF-207 | Rust UX | interactive / batch extinguishをRust CLIで実装する | CF-206, CF-164 | done | CF-207B..C; `extinguish --interactive`; `--all-matching`; `--edit-rationale`; recent evidence candidates |
...
| CF-207B | Rust Extinguish | Rust CLIでbatch extinguishとdry-run validationを実装する | CF-207A, CF-222 | done | `codefire-rs extinguish --batch <file> --dry-run --json`; strict limited YAML/JSON batch parser; `extinguish_batch_validates_all_items_before_writing_ledgers`; live batch extinguish smoke |
| CF-207C | Rust Extinguish UX | Rust CLIでinteractive plan、source/target一括解消、editor rationale、recent evidence候補を実装する | CF-207A, CF-207B, CF-226C | done | `crates/codefire-cli/src/extinguish_ux.rs`; `interactive_extinguish_plan_lists_open_fires_and_recent_evidence`; `all_matching_extinguish_extinguishes_source_target_subset`; `edit_rationale_reads_configured_editor_output` |
...
| CF-212 | Rust Server | HTTP/HTTPS server parityをRustで実装する | CF-211 | done | CF-212A..C; `cf+http://` and `cf+https://`; `serve --tls-cert --tls-key`; live self-signed HTTPS upload/list/clone smoke |
...
| CF-212B | Rust Server | Rust `serve --tls-cert --tls-key` と `cf+https://` URL parsing/client transportを実装する | CF-212A | done | `crates/codefire-cli/src/http_tls.rs`; `rustls`; `parse_https_remote_urls_and_tls_serve_args`; HTTPS client reads response by Content-Length |
| CF-212C | Rust Server | Rust HTTPS remote smokeを追加する | CF-212B | done | self-signed cert + `CODEFIRE_TLS_INSECURE=1`; live `cf+https://` upload/list/clone smoke |
| CF-213 | Rust Security | commit/request signaturesとkey rotation parityをRustで実装する | CF-212 | done | CF-213A/B; `crates/codefire-cli/src/signatures.rs`; `cargo fmt --check`; `cargo clippy --workspace --all-targets -- -D warnings`; `cargo test --workspace`; `cargo build --workspace`; file-backed/HTTP signed remote smoke |
| CF-213A | Rust Security | Rust commit HMAC signature write/verifyとkey rotation policyを実装する | CF-213 | done | `CODEFIRE_SIGNING_KEY`; `--signer`; `--key-id`; `commit_signatures.keys`; `require_history`; revoked/key validity unit tests; unsigned upload rejection smoke |
| CF-213B | Rust Security | Rust remote request HMAC signature、timestamp skew、nonce replay cacheを実装する | CF-213A | done | `CODEFIRE_REQUEST_SIGNING_KEY`; `--request-key-id`; `request_signatures.*`; timestamp skew and nonce replay unit tests; file-backed/HTTP missing-signature exit 32 and signed upload smoke |
| CF-214 | Rust Observability | perf/metrics outputをRustで実装する | CF-208, CF-166 | done | `crates/codefire-cli/src/metrics.rs`; `status/scan/verify --metrics`; JSON `data.metrics`; `metrics_attach_to_status_scan_and_verify_data`; live metrics smoke |
| CF-215 | Rust Storage | storage reportとexternal artifact refsをRustで実装する | CF-202, CF-167 | done | storage report基本版はCF-215Aでdone。artifact_ref基本版はCF-215Bでdone。remote-aware reportはCF-215Cでdone。retention/generation visibilityはCF-215Dでdone |
| CF-215A | Rust Storage | Rust CLIで `storage report` の基本集計とJSON warningを実装する | CF-202, CF-218 | done | `crates/codefire-cli/src/storage.rs`; object type/file/bytes/largest; active/idempotency bytes; large_object warning; `storage_report_counts_objects_by_type_and_warns_large_objects`; live storage report smoke |
| CF-215B | Rust Storage | external artifact ref objectとstorage report集計を実装する | CF-215A | done | `artifact_ref` object type; `.codefire/objects/artifact_refs`; storage report `external_artifacts.refs/referenced_bytes/payload_bytes_stored`; `evidence_add_records_artifact_ref_and_command_capture`; live evidence artifact smoke |
| CF-215C | Rust Storage | `storage report --remote` でfile-backed remote projectの容量を集計する | CF-215A | done | remote objects/branches/merge_requests/idempotency bytes; `storage_report_counts_file_remote_project_storage`; live remote storage report smoke |
| CF-215D | Rust Storage | remote retention policy/current generation/object generation別容量をstorage reportに出す | CF-215C | done | `server_policy.json` gc retention; `gc_state.json`; `objects_by_generation`; `storage_report_counts_file_remote_project_storage`; live remote storage report smoke |
| CF-216 | Migration | `migrate check/dry-run` を実装する | CF-209 | done | `crates/codefire-cli/src/migration.rs`; repo.json/object record/branch head sealed commit checks; planned v0.6 directory actions; `migrate_check_and_dry_run_report_compatibility_and_blockers`; live migrate smoke |
| CF-217 | Release | installerをRust binary default + Python fallbackへ切り替える | CF-216 | done | `install.sh`; Rust `codefire` default; Python `codefire-py` fallback; temp prefix install smoke |
| CF-217A | Release | `install.sh` にRust binary install pathと `codefire-py` fallback配置を実装する | CF-217 | done | `./install.sh --prefix <tmp> --completion bash --completion-dir <tmp>/completions`; `codefire --help`; `codefire-py --help`; completion file generated |
| CF-217B | Release | README/runbook/install smokeをRust defaultへ更新する | CF-217A | done | `README.md`, `docs/runbook.md`, `docs/04_cli_spec.md`, `docs/development/known-limitations.md`, `docs/development/v0.6-readiness.md`, `docs/development/v0.6-rust-rewrite-plan.md` |
...
| CF-221 | Diagnostics | status/scan/verify/doctor/storage/migrateにmachine-readable `next_actions` を追加する | CF-208, CF-220 | done | status/scan/verifyはCF-221Aでdone。storage report warning next_actionsはCF-215Aでdone。migrate next_actionsはCF-216でdone。doctor next_actionsはCF-221Bでdone |
| CF-221A | Diagnostics | Rust status/scan/verify JSONにmachine-readable `next_actions` を追加する | CF-220 | done | `status_next_actions`; `scan_next_actions`; `verification_next_actions`; `verification_next_actions_cover_blocker_classes`; live state next_actions smoke; `cargo test --workspace`; `cargo clippy --workspace --all-targets -- -D warnings` |
| CF-221B | Diagnostics | Rust `doctor --json` とmachine-readable `next_actions` を追加する | CF-216, CF-218 | done | `crates/codefire-cli/src/doctor.rs`, `crates/codefire-cli/src/doctor/checks.rs`, `crates/codefire-cli/src/doctor/report.rs`; object record integrity、branch head sealed commit、opened registry、active state JSON checks; `doctor_reports_clean_repo_and_corrupted_object_record`; `doctor_detects_invalid_branch_head_and_open_registry`; live doctor JSON/corruption smoke |
| CF-222 | Operation Plans | mutating commandに `--dry-run` / operation plan出力を追加する | CF-209, CF-218 | done | open/clone/commit/extinguish/merge/patch import/upload/request系までdone。remote mutator dry-run smoke; `cargo test --workspace`; `cargo clippy --workspace --all-targets -- -D warnings` |
| CF-222A | Operation Plans | Rust commit/extinguishに `--dry-run --json` operation planを追加する | CF-209, CF-218 | done | `codefire_operation_plan`; `commit_dry_run_returns_plan_without_updating_branch`; `extinguish_dry_run_returns_plan_without_writing_ledgers`; live commit/extinguish dry-run smoke; `cargo test --workspace`; `cargo clippy --workspace --all-targets -- -D warnings` |
| CF-222B | Operation Plans | Rust open/cloneに `--dry-run --json` operation planを追加する | CF-203, CF-210, CF-218 | done | `open_operation_plan`; `clone_operation_plan`; `open_and_clone_dry_run_return_plans_without_structural_changes`; live open/clone dry-run smoke; `cargo test --workspace`; `cargo clippy --workspace --all-targets -- -D warnings` |
| CF-222C | Operation Plans | Rust remote mutatorに `--dry-run --json` operation planを追加する | CF-130, CF-132, CF-222B | done | `upload/request-merge/request-review/request-apply --dry-run --json`; `file_remote_upload_clone_show_diff_and_merge_request_flow`; remote branch/MR no-write assertions; live remote dry-run smoke |
| CF-223 | Batch | link/fire/extinguish/evidence/artifactのbatch操作とdry-run validationを実装する | CF-207, CF-222 | done | extinguish batchはCF-223Aでdone。link batchはCF-223Bでdone。fire batchはCF-223Cでdone。evidence/artifact batchはCF-226Bでdone |
...
| CF-223B | Batch | Rust CLIで `link --batch` とdry-run all-link validationを実装する | CF-205, CF-222, CF-223A | done | `crates/codefire-cli/src/link_batch.rs`; JSON/limited YAML batch; Atom existence and duplicate validation before writing `codefire.links.yaml`; `link_batch_validates_all_items_before_writing_links`; live link batch smoke |
| CF-223C | Batch | Rust CLIでmanual `fire` と `fire --batch` dry-run all-fire validationを実装する | CF-206, CF-222, CF-223B | done | `crates/codefire-cli/src/fire.rs`; `crates/codefire-cli/src/fire/batch.rs`; Atom existence and duplicate validation before writing `fires.json`; `manual_fire_and_batch_validate_before_writing_fires`; live fire batch smoke |
| CF-224 | Safety | local/remote mutating operationにidempotency keyを実装する | CF-222, CF-211 | done | commit idempotencyはCF-224Aでdone。extinguish idempotencyはCF-224Bでdone。open/clone idempotencyはCF-224Cでdone。merge/patch import idempotencyはCF-224Dでdone。remote mutator idempotencyはCF-224Eでdone |
| CF-224A | Safety | Rust commitに `--idempotency-key` を実装する | CF-209, CF-222 | done | same key + same payload replay; same key + different payload exit 33; `commit_idempotency_key_replays_same_payload_and_rejects_conflict`; live commit idempotency smoke |
| CF-224B | Safety | Rust extinguishに `--idempotency-key` を実装する | CF-207A, CF-222, CF-224A | done | same key + same payload replay; same key + different payload exit 33; `extinguish_idempotency_key_replays_same_payload_and_rejects_conflict`; live extinguish idempotency smoke |
| CF-224C | Safety | Rust open/cloneに `--idempotency-key` を実装する | CF-222B, CF-224A | done | same key + same payload replay; same key + different payload exit 33; `open_and_clone_idempotency_replay_same_payload_and_reject_conflicts`; live open/clone idempotency smoke |
| CF-224D | Safety | Rust merge/patch importに `--idempotency-key` を実装する | CF-222, CF-224C | done | same key + same payload replay; same key + different payload exit 33; `merge_idempotency_replays_same_payload_and_rejects_conflict`; `patch_import_idempotency_replays_same_payload_and_rejects_conflict`; live merge/patch import idempotency smoke |
| CF-224E | Safety | Rust remote mutatorに `--idempotency-key` を実装する | CF-211, CF-222C, CF-224D | done | upload/request-merge/request-review/request-apply same key + same payload replay; same key + different payload exit 33; `remote_mutator_idempotency_replays_same_payload_and_rejects_conflicts`; live remote idempotency smoke |
| CF-225 | Safety | `--wait-lock` / `--lock-timeout` / JSON lock diagnosticsを実装する | CF-203, CF-219 | done | local repo lock wait/timeoutはCF-225Aでdone。remote resource lock waitとJSON envelope diagnosticsはCF-225Bでdone |
...
| CF-225B | Safety | remote mutatorに `--wait-lock` / `--lock-timeout` とJSON lock diagnosticsを実装する | CF-211, CF-219, CF-225A | done | upload/request-merge/request-review/request-apply parse lock options; remote branch/MR/project resource locks wait/timeout; `remote_resource_lock_wait_timeout_returns_owner_metadata_and_json_diagnostics`; live remote lock JSON smoke |
| CF-226 | Evidence | command outputとartifact hashを取り込むevidence capture APIを実装する | CF-215, CF-218 | done | `evidence add`基本版はCF-226Aでdone。batch evidence/artifact refsはCF-226Bでdone。remote evidence refs、verify/evidence linkageはCF-226Cでdone |
| CF-226A | Evidence | Rust CLIで `evidence add --artifact/--from-command` を実装する | CF-215B, CF-218 | done | `crates/codefire-cli/src/evidence.rs`; `artifact_ref` + `evidence` object; stdout/stderr truncation; non-zero command capture; `evidence_add_records_artifact_ref_and_command_capture`; live evidence add smoke |
| CF-226B | Evidence | Rust CLIで `evidence add --batch` とdry-run all-item validationを実装する | CF-223, CF-226A | done | JSON/limited YAML batch; artifact/cwd validation before writes; dry-run operation plan; `evidence_batch_validates_all_items_before_writing_objects`; live evidence batch smoke |
| CF-226C | Evidence | resolution evidence refs、verify linkage、remote object graph搬送を実装する | CF-211, CF-226B | done | `extinguish --evidence-ref`; `Verification.missing_evidence_refs`; `resolution_ledger -> evidence_refs -> evidence -> artifact_ref`; `extinguish_evidence_ref_links_resolution_and_verify_detects_missing_ref`; `remote_object_graph_copies_resolution_evidence_refs` |
| CF-227 | Explain | fire/atom/verify failure/storage warning向けread-only `codefire explain` を実装する | CF-220, CF-221 | done | `crates/codefire-cli/src/explain.rs`; `explain fire/atom/verify-failure/storage-warning`; `explain_fire_atom_verify_and_storage_targets_return_actions`; live explain smoke |
...
| CF-235 | Testing | diff/merge golden fixturesとGit comparison smoke testsを追加する | CF-228, CF-232 | done | `crates/codefire-cli/tests/fixtures/diff_manifest.golden`; `crates/codefire-cli/tests/fixtures/merge_dry_run.golden`; `diff_manifest_matches_golden_fixture`; `merge_dry_run_reports_plan_without_writing_target`; `codefire_text_diff_matches_git_payload_lines`; `cargo test --workspace`; `cargo clippy --workspace --all-targets -- -D warnings` |
