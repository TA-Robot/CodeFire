...
| CF-224 | Safety | local/remote mutating operationにidempotency keyを実装する | CF-222, CF-211 | todo | commit idempotencyはCF-224Aでdone。extinguish idempotencyはCF-224Bでdone。open/clone idempotencyはCF-224Cでdone。merge/patch import idempotencyはCF-224Dでdone。remote mutatorは未対応 |
