...
| CF-212B | Rust Server | Rust `serve --tls-cert --tls-key` と `cf+https://` URL parsing/client transportを実装する | CF-212A | todo | Python v0.2 `codefire serve --tls-cert --tls-key` がoracle。Rustは現状 `--tls-*` を拒否する |
| CF-212C | Rust Server | Rust HTTPS remote smokeを追加する | CF-212B | todo | self-signed cert + `CODEFIRE_TLS_INSECURE=1`; upload/list/clone/show/diff or equivalent route smoke |
