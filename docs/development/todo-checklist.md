...
| CF-221 | Diagnostics | status/scan/verify/doctor/storage/migrateにmachine-readable `next_actions` を追加する | CF-208, CF-220 | todo | status/scan/verifyはdone。storage report warning next_actionsはCF-215Aでdone。migrate next_actionsはCF-216でdone。doctorはcommand実装後に追加する |
