...
| CFB-023 | open | UX / Path Options | `status --path . --json`、`scan --path . --json`、`verify --path . --json` が `unsupported ... option: --path` で失敗する | v0.7 install後に `algorithm-evolution-agent-lab/main` を確認し、作業ディレクトリを明示してCodeFireを実行しようとしたとき | CLI利用者やAI agentが多くのコマンドで `--path` を共通指定できると期待しやすいが、実際は `scan [path]` / `verify [path]` の位置引数で、`status` はpath指定自体がないため操作が揺れる | 対象directoryに `cd` してから実行する、または `scan . --json` / `verify . --json` の位置引数を使う | `--path <path>` をstatus/scan/verify/doctorで共通aliasとして受ける、またはhelp/errorで位置引数形式を明示する |
| CFB-024 | open | UX / Help | `codefire scan --help` と `codefire verify --help` がサブコマンドhelpではなく `unsupported ... option: --help` を返す | v0.7 install後に `scan` / `verify` の正しいpath指定を確認しようとしたとき | AI agentが使い方を自己修正する導線が弱く、誤ったoptionを試した後に回復しづらい | top-level `codefire help` を読む | 全サブコマンドで `-h` / `--help` を受け、該当コマンドのusage、path指定、json/metrics等のoptionを表示する |
