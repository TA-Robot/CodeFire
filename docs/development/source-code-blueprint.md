...
`source-code-map.md` はfile索引、`source-code-mental-model.md` は処理の流れ、`source-code-reconstruction-guide.md` はsourceを開く前に実装の形を再構成するガイド、`source-data-model-catalog.md` は型とJSONの辞書、`source-code-anatomy.md` は主要fileの内部構造、`command-source-trace.md` はcommand別traceである。この文書はそれらを統合し、「この機能はどの層に存在し、どのfileをどう変えるべきか」を示す。
